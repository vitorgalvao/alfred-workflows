...
$LOAD_PATH.unshift "#{Redcarpet_dir}/gems/rouge-3.3.0/lib"
...
begin
  ...
  notification('Reinstalling converter and syntax highlighter', 'Please wait a few seconds')

  require 'fileutils'
  FileUtils.rm_r(Redcarpet_dir) if Dir.exist?(Redcarpet_dir)

  if system('/usr/bin/gem', 'install', '--no-document', '--install-dir', Redcarpet_dir, 'redcarpet', 'rouge', out: File::NULL)
